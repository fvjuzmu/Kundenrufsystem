...
function addToOrderList( orderID )
{
    ...
    // if the list has more than 10 elements, then remove the oldest one
    var orderListElements = orderList.find('li' );
    if ( orderListElements.length > 10 )
    {
        orderListElements.eq( 10 ).remove();
    }
...
function newOrderID( keyPress )
{
    ...
    $.post( "http://" + server + "/krs/api/saveNewOrder.php", postData, function ( returnData )
    {
        ...
        // check if the return data has a success message
